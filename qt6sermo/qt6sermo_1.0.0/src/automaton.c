...
create_gvim(AttributeSet * Attr)
{
	...
	g_snprintf(socket_id, sizeof(socket_id), "%d", gtk_socket_get_id (GTK_SOCKET(socket)));
...
/* GTK3: always available - removed version guard */
	...
	/* Enregistrer space-expand pour le moteur de disposition (widget_hbox/vbox)
		...
		tag_attr *attributes)
{
	...
	case 0:
		...
		g_free(inst.argument); inst.argument = g_strdup_printf("%s", argument);
		break;
	case 1:
		/*
