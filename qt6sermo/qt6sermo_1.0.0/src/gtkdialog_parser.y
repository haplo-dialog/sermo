...
{
	if (!option_print_ir) {
		/* run_program() lance la boucle d'événements Qt (gtk_main →
		 * QApplication::exec) et revient à la fermeture. Il NE FAUT PAS
		 * rendre la main à yyparse() (programme terminé) : sinon l'analyseur
		 * poursuit après la règle « window » → « syntax error near <window> »
		 * ET le démontage de QApplication plante (QThreadStorage / SIGSEGV).
		 * On quitte donc après la boucle, comme la branche --print-ir. */
		run_program();
		/* Sortie robuste : la sortie attendue est déjà émise ; _Exit() évite
		 * le démontage de QApplication (avertissements QThreadStorage, voire
		 * SIGSEGV au teardown). */
		fflush(NULL);
		_Exit(EXIT_SUCCESS);
	} else {
