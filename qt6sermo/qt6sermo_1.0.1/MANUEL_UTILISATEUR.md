# Manuel Utilisateur — qt6sermo (qt6sermo)
...
`qt6sermo` est un utilitaire en ligne de commande qui permet de créer des **fenêtres graphiques Qt6** depuis n'importe quel script shell, Python, Perl ou autre langage interprété — sans écrire une seule ligne de code Qt6.

Le principe est simple : vous décrivez votre interface en XML, et qt6sermo l'affiche. Quand l'utilisateur ferme la fenêtre, les valeurs saisies sont renvoyées sur la sortie standard sous forme de variables shell.
...
qt6sermo --program=DIALOG
# Sortie : NOM="Jean"  EXIT="ok"
```

### 1.1 Différences avec gtkdialog 0.8.3 (origine)

qt6sermo est **100% compatible** avec les scripts écrits pour la version 0.8.3. Les seules différences visibles sont :

- L'apparence suit le thème Qt6 du système (style natif du bureau)
- Les couleurs sont spécifiées au format `#RRGGBB`
- Qt6 fournit un rendu haute qualité et le support HiDPI
...
sudo apt-get install cmake qt6-base-dev \
  flex bison pkg-config gcc g++

# 2. Compiler (voir COMPILE.md pour le détail)
cd qt6sermo_1.0.0/
...
```bash
echo '<window><vbox>
  <text><label>qt6sermo fonctionne !</label></text>
  <button><label>Fermer</label><action>EXIT:ok</action></button>
</vbox></window>' | qt6sermo --stdin
```

Une fenêtre Qt6 doit apparaître. Si elle s'ouvre, l'installation est réussie.

### 2.3 Désinstallation

Le projet est construit avec CMake, qui ne fournit pas de cible `uninstall` :
on supprime les fichiers listés dans le manifeste d'installation généré au
moment du `cmake --install` :
...
Si qt6sermo a été installé via le paquet `.deb`, utiliser plutôt
`sudo apt remove qt6sermo`.
...
qt6sermo accepte son XML de trois façons :

**Depuis stdin :**
```bash
echo '<window>...</window>' | qt6sermo --stdin
```

**Depuis une variable d'environnement :**
```bash
export MON_DIALOG='<window>...</window>'
qt6sermo --program=MON_DIALOG
```

**Depuis un fichier :**
```bash
qt6sermo --file=mon_interface.xml
```

### 3.2 Récupérer les valeurs saisies

Quand l'utilisateur ferme la fenêtre, qt6sermo affiche sur stdout les valeurs de tous les widgets nommés. Pour les utiliser dans votre script :
...
### 4.1 Structure d'un document qt6sermo
...
Tout document qt6sermo commence par `<window>`. Les widgets sont imbriqués dans des conteneurs (`<vbox>`, `<hbox>`, `<frame>`, `<notebook>`).
...
La valeur initiale peut aussi être donnée par l'attribut `range-value`
(prioritaire sur `value` ; `<default>` puis `<input>` priment si présents) :
`<hscale range-min="0" range-max="100" range-value="42">`.
...
<timer milliseconds="1000" visible="false">
  ...
  <action>date >> /tmp/ticks.log</action>
</timer>
```

Déclenche ses actions à chaque tick. La période se règle avec `interval`
(secondes, décimales acceptées : `interval="0.5"`) ou `milliseconds`
(millisecondes). La variable exportée contient le **nombre de ticks écoulés**.

Un `<timer>` accepte **plusieurs** `<action>`, traitées exactement comme
celles d'un `<button>` : les directives `EXIT:valeur`, `REFRESH:WIDGET`,
`Command:cmd`, etc. sont interprétées comme des actions (et non passées
telles quelles au shell) ; tout texte sans préfixe reconnu est exécuté
comme commande shell. Les attributs `type="exit"` (ou `function="exit"`)
sont également honorés :

```xml
<!-- Ferme automatiquement le dialogue après 10 s avec EXIT="timeout" -->
<timer interval="10" visible="false">
  <action type="exit">timeout</action>
</timer>
```

C'est le mécanisme utilisé par la suite `tests/behaviour/` pour auto-fermer
les fenêtres de test (`<action>EXIT:ok</action>`).
...
### 5.13 Widgets additionnels (1.0.0)

Les six widgets suivants ont été ajoutés dans la version 1.0.0.
Ils étendent le jeu de widgets hérité de gtkdialog 0.8.x.
...
qt6sermo ouvre l'URI dans le navigateur par défaut (via QDesktopServices).
...
À la fermeture, qt6sermo émet sur stdout : `MA_VALEUR="contenu saisi"`
...
```bash
qt6sermo --include=/chemin/fonctions.sh --program=DIALOG
```
...
eval $(echo "$CONFIRM" | qt6sermo --stdin)
...
eval $(echo "$SELECTOR" | qt6sermo --stdin)
...
eval $(qt6sermo --program=FORMULAIRE)
...
eval $(echo "$ONGLETS" | qt6sermo --stdin)
```
...
```bash
#!/bin/bash
# mon_app.sh — Application qt6sermo
...
eval $(qt6sermo --include="$0" --program=INTERFACE)
...
qt6sermo --program=FENETRE_PRINCIPALE
```
...
R : Utilisez le format de couleur `#RRGGBB`. La syntaxe XML et le comportement fonctionnel sont identiques à gtkdialog 0.8.3.

**Q : Le widget `<terminal>` ne s'affiche pas**  
R : Sous Qt6, le terminal embarqué utilise les classes de processus Qt ; aucune dépendance VTE n'est requise.
...
**Q : Puis-je utiliser qt6sermo en Python ?**  
R : Oui. Construisez la chaîne XML en Python et passez-la via `subprocess.run(['qt6sermo', '--stdin'], input=xml_str)`.

**Q : Comment déboguer une interface qui ne se comporte pas comme prévu ?**  
R : Lancez avec `qt6sermo --debug --program=VAR` pour activer les messages de diagnostic. Redirigez stderr : `qt6sermo --program=VAR 2>debug.log`.
