#!/bin/sh
# autogen.sh — gtk3dialog 1.0.0 (haplo-dialog)
#
...
#   1. creates a clean symlink  /tmp/gtk3dialog-src  →  <real srcdir>
#   2. runs configure from a dedicated build directory  /tmp/gtk3dialog-build/
...
case "$srcdir" in
    *\ *|*\&*|*\(*|*\)*|*\;*|*\|*|*\<*|*\>*|*\`*|*\$*)
        safe_srcdir="/tmp/gtk3dialog-src"
        rm -f "$safe_srcdir"
...
if [ "$safe_srcdir" != "$srcdir" ] || [ -n "$GTK3DIALOG_BUILDDIR" ]; then
    builddir="${GTK3DIALOG_BUILDDIR:-/tmp/gtk3dialog-build}"
    mkdir -p "$builddir"
    ...
    if [ -f "$srcdir/config.status" ] || \
       [ -f "$srcdir/src/gtk3dialog" ] || \
       [ -f "$srcdir/src/gtk3dialog.o" ]; then
        ...
        find "$srcdir/src" -maxdepth 1 \
             \( -name "*.o" -o -name "*.lo" -o -name "*.a" \
                -o -name "gtk3dialog" -o -name "gtk3dialog.exe" \) \
