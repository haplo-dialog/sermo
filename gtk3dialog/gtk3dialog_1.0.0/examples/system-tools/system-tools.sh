#!/bin/bash
# ============================================================================
# Outils Système, exemple complet pour gtk3dialog
#
# Version   : 4.0
# Interface : gtk3dialog 1.0.0 (alias gtkdialog accepté)
...
#     façon propre de programmer avec gtk3dialog, sans « export -f » ;
...
#   - « set -e » et « trap ERR » sont volontairement absents : gtk3dialog
#     renvoie un code non nul à la fermeture normale de la fenêtre.
#
# Dépendances : gtk3dialog, pkexec, et un terminal + un éditeur graphique
...
# Sortie sur stdout, pour alimenter un <input> de gtk3dialog.
if [[ "${1:-}" == "--print" ]]; then
    case "${2:-}" in
...
handle_action() {
    ...
    case "$action" in
        ...
        about)
            cat > "$tmp" <<'ABOUT'
Outils Systeme, exemple complet pour gtk3dialog
===============================================
Version   : 4.0
Interface : gtk3dialog 1.0.0 (l'alias gtkdialog fonctionne aussi)
...
Le motif « --do » :
  gtk3dialog execute une <action>, celle-ci rappelle ce meme script avec
  ...
  C'est la facon propre d'ecrire un programme gtk3dialog un peu serieux.
...
Ce qui est reellement fait cote securite :
  ...
  - set -e et trap ERR sont absents : gtk3dialog renvoie un code non nul
  ...
  <vbox spacing="4" space-expand="true" space-fill="true">
    ...
    <hbox spacing="6" space-expand="false" space-fill="false">
      ...
      <text space-expand="true" space-fill="true">
        <label>Exemple gtk3dialog 1.0.0</label>
      </text>
      <button space-expand="false" space-fill="false">
        <label>Quitter</label>
...
check_deps() {
    local missing=()
    command -v gtk3dialog &>/dev/null || command -v gtkdialog &>/dev/null \
        || missing+=("gtk3dialog")
...
# NE PAS utiliser set -e ici : gtk3dialog renvoie un code non-zéro normal
# à la fermeture de la fenêtre, ce qui déclencherait un arrêt prématuré.
gtk3dialog --program=MAIN_DIALOG --center
