#!/bin/bash
# run_tests.sh — Suite de tests fonctionnels gtk3dialog 1.0.0
#
# Usage:  ./run_tests.sh [/path/to/gtk3dialog]
#
# Teste le binaire gtk3dialog de façon non-interactive (sans affichage réel) :
...
# Configuration
# ---------------------------------------------------------------------------
GTKDIALOG="${1:-$(command -v gtk3dialog 2>/dev/null || echo "")}"
...
# Run gtk3dialog with a timeout; capture stdout+stderr.
# Returns exit status of gtk3dialog (or timeout exit = 124).
run_gtkd() {
    local xml="$1"
    local extra_args="${2:-}"
    if [[ -z "$GTKDIALOG" ]]; then
    ...
    # We use DISPLAY="" on purpose: gtk3dialog will fail to open a window
...
# by checking the format conventions used in gtk3dialog XML <action> tags.
