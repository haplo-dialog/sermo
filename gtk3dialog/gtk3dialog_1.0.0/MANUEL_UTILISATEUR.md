...
`gtk3dialog` est un utilitaire en ligne de commande qui permet de créer des **fenêtres graphiques GTK 3** depuis n'importe quel script shell, Python, Perl ou autre langage interprété — sans écrire une seule ligne de code GTK.

Le principe est simple : vous décrivez votre interface en XML, et gtk3dialog l'affiche. Quand l'utilisateur ferme la fenêtre, les valeurs saisies sont renvoyées sur la sortie standard sous forme de variables shell.
...
### 1.1 Différences avec gtk3dialog 0.8.3

gtk3dialog est **100% compatible** avec les scripts écrits pour la version 0.8.3. Les seules différences visibles sont :
...
```bash
echo '<window><vbox>
  <text><label>gtk3dialog fonctionne !</label></text>
...
gtk3dialog accepte son XML de trois façons :

**Depuis stdin :**
```bash
echo '<window>...</window>' | gtk3dialog --stdin
```

**Depuis une variable d'environnement :**
```bash
export MON_DIALOG='<window>...</window>'
gtk3dialog --program=MON_DIALOG
```

**Depuis un fichier :**
```bash
gtk3dialog --file=mon_interface.xml
```

### 3.2 Récupérer les valeurs saisies

Quand l'utilisateur ferme la fenêtre, gtk3dialog affiche sur stdout les valeurs de tous les widgets nommés. Pour les utiliser dans votre script :
...
eval $(gtk3dialog --program=DIALOG)
...
### 4.1 Structure d'un document gtk3dialog
...
Tout document gtk3dialog commence par `<window>`. Les widgets sont imbriqués dans des conteneurs (`<vbox>`, `<hbox>`, `<frame>`, `<notebook>`).
...
Ils sont natifs GTK3 et n'existent pas dans gtk3dialog 0.8.x.
...
À la fermeture, gtk3dialog émet sur stdout : `MA_VALEUR="contenu saisi"`
...
```bash
gtk3dialog --include=/chemin/fonctions.sh --program=DIALOG
```
...
eval $(echo "$CONFIRM" | gtk3dialog --stdin)
...
eval $(echo "$SELECTOR" | gtk3dialog --stdin)
...
eval $(gtk3dialog --program=FORMULAIRE)
...
eval $(echo "$ONGLETS" | gtk3dialog --stdin)
```
...
```bash
#!/bin/bash
# mon_app.sh — Application gtk3dialog
...
eval $(gtk3dialog --include="$0" --program=INTERFACE)
...
gtk3dialog --program=FENETRE_PRINCIPALE
```
...
**Q : Mon script fonctionnait avec gtk3dialog 0.8.3 mais plus maintenant**  
...
**Q : Puis-je utiliser gtk3dialog en Python ?**  
R : Oui. Construisez la chaîne XML en Python et passez-la via `subprocess.run(['gtk3dialog', '--stdin'], input=xml_str)`.

**Q : Comment déboguer une interface qui ne se comporte pas comme prévu ?**  
R : Lancez avec `gtk3dialog --debug --program=VAR` pour activer les messages de diagnostic. Redirigez stderr : `gtk3dialog --program=VAR 2>debug.log`.
