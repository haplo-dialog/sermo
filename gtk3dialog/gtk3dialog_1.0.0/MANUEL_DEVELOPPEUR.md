# Manuel Développeur et Mainteneur — gtk3dialog
...
gtk3dialog est un programme C de ~35 000 lignes organisé en trois couches distinctes :

```
┌─────────────────────────────────────────────────────────┐
│                    COUCHE D'ENTRÉE                       │
│   gtk3dialog.c  ← arguments CLI, lecture XML             │
...
Exemple :
```bash
GTK_DEBUG=interactive G_DEBUG=fatal_criticals ./src/gtk3dialog --stdin < examples/button/button
```
...
```
src/
├── gtk3dialog.c/.h         # Point d'entrée main(), parsing des options CLI
...
├── gtkdialog_parser.y     # Grammaire Bison (syntaxe gtk3dialog XML)
...
Le parser n'est **pas** un vrai parser XML : il utilise une grammaire Bison spécifique au format gtk3dialog. Les tokens reconnus par le lexer sont les balises XML gtk3dialog, pas du XML générique.
...
commande → strpbrk(cmd, "|&;<>(){}$`\"'~*?")
  ...
  exec direct — sans shell          g_debug() "shell fallback for …"
...
> `G_MESSAGES_DEBUG=all ./gtk3dialog …`
...
`signals.c` fait le lien entre les signaux GTK (événements bas niveau) et le système d'actions gtk3dialog haut niveau.
 ...
 * gtk3dialog — fork modernisé de gtk3dialog 0.8.3
...
Pour activer les messages `g_debug()` : `G_MESSAGES_DEBUG=all ./gtk3dialog …`
...
2. Ajouter `$(FOO_CFLAGS)` à `gtk3dialog_CFLAGS` dans `src/Makefile.am`
3. Ajouter `$(FOO_LIBS)` à `gtk3dialog_LDADD` dans `src/Makefile.am`
4. Ajouter `libfoo-dev` dans `Build-Depends` de `debian/control`
5. Relancer `./autogen.sh`

### 7b.5 Page de manuel

La page de manuel est dans `src/gtk3dialog.1` (format troff). Elle est automatiquement installée par `make install` via `dist_man_MANS` dans `src/Makefile.am`. Pour la consulter sans installer :

```bash
man ./src/gtk3dialog.1
```
...
src/Makefile.am         — 6 paires .c/.h dans gtk3dialog_SOURCES
...
Makefile  →  [make]  →  binaire gtk3dialog
```
...
for example in examples/*/; do
    ...
    echo "=== Test $name ===" 
    timeout 2 ./src/gtk3dialog --file="$example/$(ls $example | head -1)" \
...
# Tester
ASAN_OPTIONS=detect_leaks=1 ./src/gtk3dialog --stdin < examples/button/button
```
...
```bash
GTK_DEBUG=interactive ./src/gtk3dialog --stdin < examples/notebook/notebook
...
valgrind ./src/gtk3dialog --stdin < examples/button/button
...
# 2. Checksec
checksec --file=src/gtk3dialog | grep -E "Full|enabled"

# 3. Analyse ASAN
make CFLAGS="-fsanitize=address,undefined -g" && \
  for ex in examples/*/; do
    echo -n "$ex: "
    ASAN_OPTIONS=detect_leaks=0 timeout 2 ./src/gtk3dialog \
...
sed -i 's/AC_INIT(\[gtk3dialog\], \[0\.9\..*\]/AC_INIT([gtk3dialog], [0.9.1]/' configure.ac
