...
> [v1.1.1](https://gitlab.com/haplo-dialog/sermo/-/releases/v1.1.1) release
...
[![Version](https://img.shields.io/badge/version-1.1.1-informational.svg)](CHANGELOG.en.md)
...
The packages are attached to the [**v1.1.1**](https://gitlab.com/haplo-dialog/sermo/-/releases/v1.1.1) release, with their checksums.
There is **no APT repository**: you download, you verify, you install.

```sh
U=https://gitlab.com/api/v4/projects/85674825/packages/generic/sermo/1.1.1
for f in gtk3sermo_1.1.1-1_amd64.deb gtksermo_1.1.1-1_all.deb SHA256SUMS; do
    curl -fLO "$U/$f"
done

sha256sum --ignore-missing -c SHA256SUMS
sudo apt install ./gtk3sermo_1.1.1-1_amd64.deb
```

| Package | Command installed | Conflicts |
|---|---|---|
| `gtk3sermo` 1.1.1-1 | `/usr/bin/gtk3sermo` | none |
| `gtk4sermo` 1.1.1-1 | `/usr/bin/gtk4sermo` | none |
| `gtksermo` 1.1.1-1 | `/usr/bin/gtkdialog` | **with `gtkdialog` and `gtk3dialog`** |
...
> replace it** with the matching package from `v1.1.1`.
>
> Release `v1.0.0-4` does carry the three fixes: its packages are sound, merely
> superseded by those of `v1.1.1`.
...
cd sermo/gtk3sermo/gtk3sermo_1.1.1        # or gtk4sermo/gtk4sermo_1.1.1
...
- User/developer manuals (`gtk3sermo/gtk3sermo_1.1.1/MANUEL_*.md`).
- Man pages `gtk3sermo(1)` and XML reference `haplo-dialog-xml(5)`.
- Texinfo manuals (`gtk3sermo/gtk3sermo_1.1.1/doc/`).
