...
/*
 * Global variables declared extern in gtkdialog.h — defined here once.
 */
...
"See the gtk4sermo(1) manual page for more information."
	...
	/* GTK4: gtk_get_option_group() was removed; GTK no longer exposes its
	 * own option group, so there is nothing to add here. */
...
main(int argc, char *argv[])
{
	...
	/*
	 * If we have an event driven program we recall the gtk4sermo with
...
gtkdialog_initialized:
	 ...
	 * affichage n'est requis. gtk_init() avorte sans serveur X ; on utilise
	 * donc gtk_init_check() (non fatal) dans ce cas, comme gtk3sermo. */
	if (option_print_ir)
