...
void action_closewindow(GtkWidget *widget, char *string)
{
	...
	if (existing != NULL && existing->Widget != NULL) {
		...
		/* If we are closing the last window then we can exit gtk4sermo */
...
void action_launchwindow(GtkWidget *widget, char *string)
{
	...
	if (var->Type != WIDGET_TIMER) {
		 ...
		 * (see Issue16 on the gtk4sermo website) */
...
{
	/*
	 ** GTK4 port: GtkFileSelection was removed in GTK4. File selection is
	 ** now handled by GtkFileChooser (see the fileselect action above), so
	 ** this legacy callback is retained only for ABI compatibility and no
	 ** longer reads a filename from the obsolete GtkFileSelection widget.
	 */
