...
/*
 * window_close_request_handler:
 * GTK4 stand-in for the GTK3 "delete-event" connection.  GtkWindow emits
 * close-request with one argument fewer, so this adapts the shape and
 * defers to the historical handler, which is left untouched.
 */
				...
				if ((fd = inotify_init()) != -1) {
					...
					if ((wd = inotify_add_watch(fd, (gchar*)filename,
						...
						if (!count) {
							 ...
							 * being routed through gtk4sermo's signal handling
