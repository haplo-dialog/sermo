	...
	AttributeSet *Attr, tag_attr *attr, gint Type)
{
	...
	for (count = 0; count < elements.nwidgets; count++) {
		...
		if (labels && labels->n_lines > count)
			g_snprintf(text, sizeof(text), "%s", labels->line[count]);
		else if (tab_prefix && tab_suffix)
			g_snprintf(text, sizeof(text), "%s%i%s", tab_prefix, count + tab_base_index, tab_suffix);
		else if (tab_prefix)
			g_snprintf(text, sizeof(text), "%s%i", tab_prefix, count + tab_base_index);
		else if (tab_suffix)
			g_snprintf(text, sizeof(text), "%i%s", count + tab_base_index, tab_suffix);
		else
			g_snprintf(text, sizeof(text), "Page %i", count + tab_base_index);
...
gchar *widget_notebook_envvar_construct(GtkWidget *widget)
{
	...
	g_snprintf(envvar, sizeof(envvar), "%i", gtk_notebook_get_current_page(GTK_NOTEBOOK(widget)));
