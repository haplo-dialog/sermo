...
#else
/* GTK4_TODO: WIDGET_GVIM uses GtkSocket (X11-only, removed in GTK4).
 * A proper GTK4 implementation would embed gvim via XdgActivation or
 * a Wayland-compatible embedding protocol. */
static GtkWidget *
create_gvim(AttributeSet * Attr)
{
	fprintf(stderr, "%s(): GTK4_TODO -- GtkSocket removed in GTK4; "
	                "GVIM embedding not available.\n", __func__);
	GtkWidget *stub = gtk_label_new("[gvim -- socket embedding not available in GTK4]");
	gtk_widget_set_visible(stub, TRUE);
	return stub;
}
