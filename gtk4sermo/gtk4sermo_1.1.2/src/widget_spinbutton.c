...
void widget_spinbutton_refresh(variable *var)
{
	...
	if (!initialised) {
		...
		/* PAS de "icon-press"/"icon-release" ici.
		 *
		 * En GTK 3, GtkSpinButton derive de GtkEntry et herite donc de ses
		 * signaux d'icone. En GTK 4 ce n'est plus le cas : GtkSpinButton est
		 * un GtkWidget qui implemente GtkEditable, et ces deux signaux n'y
		 * existent pas. Les brancher quand meme faisait imprimer a GTK, a
		 * CHAQUE ouverture d'un dialogue contenant un <spinbutton> :
		 *   GLib-GObject-CRITICAL: signal 'icon-press' is invalid for
		 *   instance ... of type 'GtkSpinButton'
		 * Mesure du 2026-08-27 : 2 avertissements pour <spinbutton>, 0 pour
		 * <entry> et 0 pour <entry password="true"> -- GtkEntry, lui, a bien
		 * garde ces signaux en GTK 4. Le defaut venait d'un bloc recopie du
		 * port GTK 3 sans etre reverifie (son commentaire disait encore
		 * "GTK3: always available").
		 */
	}
