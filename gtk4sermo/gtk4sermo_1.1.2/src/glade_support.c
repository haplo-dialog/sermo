...
/* GTK4: GtkEntry's "populate-popup" signal and GtkMenu were removed. */
		...
		const gchar *window_name) 
{
	...
	/*
	 * GTK4: gtk_builder_connect_signals_full() and GtkBuilderConnectFunc
	 * were removed. GtkBuilder now resolves handlers automatically through
	 * its GtkBuilderScope at load time, so the explicit per-handler
	 * connection pass below is no longer available. The custom dispatcher
	 * signal_handler_connector() is retained for reference but is not
	 * invoked by the builder in GTK4.
	 */
...
#endif	/* GtkBuilder always available in GTK4 */
