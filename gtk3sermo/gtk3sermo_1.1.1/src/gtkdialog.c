...
#include "safe_exec.h"
/*
 * gtk3sermo.c: Starting up the program.
...
"See the gtk3sermo(1) manual page for more information."
);
...
get_program_from_file(char *name)
{
	 ...
	 * (e.g. #!/usr/bin/gtk3sermo), then reopen the file from the beginning
	 * so the parser sees the full XML content.  The gtk3sermo grammar does
	 * not include a '#!' token, so this pre-pass is required.
	 */
	program_src = NULL;
	result = getline(&program_src, &tmp, sourcefile);
	if (result > 0 && program_src[0] != '#') {
		fclose(sourcefile);
		sourcefile = fopen(name, "r");
		if (!sourcefile) 
			g_error("Error opening file '%s': %m", name );
...
print_version_exit(int exitcode)
{
	...
	/* L'ordre des mots est FIGE : la version doit rester le 3e mot.
	 * Des scripts la lisent par position -- deux exemples livres le font
	 * (examples/pfeme/main, examples/pfontview/main : funcAppVersionGet
	 * "$GTKDIALOG -v" 2). La sortie de --version fait partie de la surface
	 * publique au sens de VERSIONING.md : en changer la forme est une rupture
	 * de compatibilite, pas un correctif. Mesure du 2026-08-27 : deplacer la
	 * version en 4e position a fait tomber 2 exemples par port. */
...
main(int argc, char *argv[])
{
	...
	/*
	 * If we have an event driven program we recall the gtk3sermo with
...
gtkdialog_initialized:
	/* En mode --print-ir on ne fait qu'analyser le XML puis sortir : aucun
	 * affichage n'est requis. gtk_init() avorte (SIGABRT) sans serveur X, ce
	 * qui empêche toute validation du parsing en CI headless. On utilise donc
	 * gtk_init_check() (non fatal) dans ce cas ; le parser appelle ensuite
	 * print_program() puis exit(0) sans jamais construire de widgets. */
	if (option_print_ir)
