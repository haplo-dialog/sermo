...
# Un binaire introuvable est un ECHEC, pas un saut. Jusqu'au 2026-08-27 ces deux
# cas etaient comptes « skip » : make check restait vert alors qu'aucune epreuve
# binaire n'avait tourne. Un banc qui ne peut pas echouer ne sert a rien.
if [[ -z "$GTKDIALOG" ]]; then
    fail "aucun binaire a eprouver : ni argument, ni gtk3sermo dans le PATH"
    echo "  Usage : $0 /chemin/vers/gtk3sermo" >&2
    GTKDIALOG_AVAILABLE=0
elif [[ ! -x "$GTKDIALOG" ]]; then
    fail "$GTKDIALOG n'est pas executable"
    GTKDIALOG_AVAILABLE=0
else
...
if [[ "$GTKDIALOG_AVAILABLE" -eq 0 ]]; then
    ...
    # 2.1 Fenêtre minimale valide — PREUVE POSITIVE exigée.
    #
    # Jusqu'au 2026-08-27 cette épreuve ne pouvait pas échouer : elle ne cherchait
    # que les mots « parse error » dans la sortie, si bien qu'un binaire qui
    # n'imprimait RIEN les passait toutes. Sabotage mesuré : un faux binaire
    # réduit à « exit 1 » donnait 49 passés / 0 échoué. On exige maintenant que la
    # représentation interne soit réellement produite.
    ...
    elif ! echo "$OUT" | grep -q "push(new window("; then
        fail "Fenêtre minimale : aucune représentation interne produite (sortie vide ou muette)"
    elif ! echo "$OUT" | grep -q "push(new button("; then
        fail "Fenêtre minimale : le bouton n'apparaît pas dans la représentation interne"
    else
        pass "Fenêtre minimale : représentation interne produite (window + button)"
    fi

    # 2.2 Balise inconnue → refus NOMMÉ, sans crash.
    #
    # Comportement mesuré le 2026-08-27 : l'analyseur s'arrête et écrit
    # « Error in line N, near token '<...>': Unknown tag. » — il ne produit AUCUNE
    # représentation interne. C'est le bon comportement : il ne l'ignore pas en
    # silence. On exige donc l'erreur nommée, pas une représentation.
    ...
    if echo "$OUT" | grep -qi "Segmentation\|Aborted"; then
        fail "Balise inconnue : crash"
    elif ! echo "$OUT" | grep -qi "Unknown tag"; then
        fail "Balise inconnue : acceptée en silence, aucune erreur nommée"
    else
        pass "Balise inconnue : refusée proprement, erreur nommée"
    fi

    # 2.3 XML malformé → l'erreur doit être NOMMÉE, et sans crash.
    ...
    elif ! echo "$OUT" | grep -qi "syntax error\|Error in line"; then
        fail "XML malformé : accepté en silence, aucune erreur signalée"
    else
        pass "XML malformé : refusé proprement, erreur nommée"
    fi
