...
/* Wrapper GFunc (evite -Wcast-function-type sur gtk_tree_path_free) */
static void _gtkd_tree_path_free_gfunc(gpointer data, gpointer user_data) {
	(void)user_data;
...
gchar *widget_tree_envvar_construct(GtkWidget *widget)
{
	...
	if (selectionmode == GTK_SELECTION_NONE) {
		...
		if (gtk_tree_selection_count_selected_rows(selection)) {
			...
			g_list_foreach(selectedrows, _gtkd_tree_path_free_gfunc, NULL);
		...
		if (column_sort_function && index < column_sort_function->n_lines) {
			...
			if (function == 1 || function == 2) {
				if ((gtk_tree_model_get_column_type(GTK_TREE_MODEL(store),
					...
					g_debug( "%s(): column-sort-function: natural \
sorting is compatible only with columns of type string.", __func__);
				}
			}
		}
	}
	...
	gint command_or_file)
{
	...
	if (infile) {
		...
		while (fgets(oneline, 512, infile) != NULL) {
			...
			for (count = (gint)strlen(oneline) - 1; count >= 0; count--)
