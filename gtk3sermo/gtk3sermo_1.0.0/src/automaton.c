...
instruction_execute(instruction command)
{
	...
	case IMASSG:
		{
		/* Imperative assignment: pop value from stack and set as env var. */
		stackelement s;
		s = pop();
	...
	AttributeSet *Attr, tag_attr *attr, gint Type)
{
	...
	switch (Type) {
		...
		case WIDGET_TERMINAL:
			...
			/* vte_terminal_get_padding() removed in VTE 0.36; padding is
			 * handled internally since then — assume zero padding. */
			xpad = 0;
			ypad = 0;
...
/*
 * create_gvim — EXPERIMENTAL / UNTESTED (R9)
 *
 * Embeds a gvim instance into a GtkSocket/GtkWindow popup.
 * This function is intentionally kept for GtkSocket embedding support
 * but has never been formally tested. It must NOT be removed because
 * WIDGET_GVIM is part of the automaton dispatch table.
 *
 * Known issues:
 *   - gtk_widget_set_extension_events() was removed in GTK3 (already fixed).
 *   - The commented-out configure-event handler was never wired up properly.
 *   - No test coverage exists for this code path.
 *
 * To enable testing, compile with --enable-debug and set the GVIM_SOCKET
 * environment variable to the target socket ID.
 *
 * TODO (v0.9.1): add a formal test in tests/ and remove this warning
 * once the widget behaves correctly end-to-end.
 */
static GtkWidget *
create_gvim(AttributeSet * Attr)
{
	...
	/* gtk_widget_set_extension_events() removed in GTK3 — no replacement needed */
	char socket_id[32];
	...
	g_snprintf(socket_id, sizeof(socket_id), "%lu", (unsigned long) gtk_socket_get_id (GTK_SOCKET(socket)));
...
*/
	...
	while (act != NULL) {
		...
		/* gtk_signal_connect/GTK_SIGNAL_FUNC removed in GTK3 */
...
/* GTK3: always available - removed version guard */
	...
	case WIDGET_GVIM:
		/* EXPERIMENTAL — see create_gvim() comment above (R9) */
...
void _more_memory_needed(void)
{
	...
	if (program == NULL) {
		memory_counter = 128;
		memory = g_malloc(memory_counter * sizeof(instruction));  /* was: malloc() — g_malloc() aborts on OOM */
	} else {
		memory_counter *= 2;
		memory =
	...
	if (memory == NULL) {
		g_debug( "%s: not enought memory.", __func__);
		exit(EXIT_FAILURE);
	}
#ifdef DEBUG
		...
		tag_attr *attributes)
{
	...
	case 1:
		 ...
		 */
		g_free(inst.argument); inst.argument = g_strdup_printf("Command:%s", argument);
		break;
	case 2:
		/* 
		 ** The text argument is a file name.
		 */
		g_free(inst.argument); inst.argument = g_strdup_printf("File:%s", argument);
		break;
	case 3:
		/* 
		 ...
		 ** cleared.
		 */
		g_free(inst.argument); inst.argument = g_strdup_printf("Clear:%s", argument);
		break;
	case 4:
		/*
		**
		*/
		g_free(inst.argument); inst.argument = g_strdup_printf("Append:%s", argument);
		break;
	case 10:
		/* 
		 ...
		 */
		g_free(inst.argument); inst.argument = g_strdup_printf("Fileselect:%s", argument);
		break;
	case 11:
		/* 
		 ...
		 */
		g_free(inst.argument); inst.argument = g_strdup_printf("Removeselected:%s", argument);
		break;
	case 12:
		/* 
		 ** The text argument is widget name and we have to remove the
		 ** selected element from it.
		 */
		g_free(inst.argument); inst.argument = g_strdup_printf("Refresh:%s", argument);
		break;
	case 13:
		/* 
		 ...
		 **  
		 */
		g_free(inst.argument); inst.argument = g_strdup_printf("Launch:%s", argument);
		break;
	default:
		g_warning( "%s(): Warning: Unknown sub-attribute %d",
...
int token_store_with_argument(
		...
		const char *argument)
{
	/* new_tag_attributeset() g_strdup()s both arguments internally, so pass
	 * them directly — the previous strdup()s were leaked (never freed). */
	tag_attr *attributes = new_tag_attributeset("text", (char *)argument);
