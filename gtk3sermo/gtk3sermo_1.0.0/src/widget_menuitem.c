	...
	AttributeSet *Attr, tag_attr *attr, gint Type)
{
	...
	GtkWidget        *widget;
	gchar             accel_path[64];
	gchar            *active = "false";  /* défaut sûr si aucun attribut checkbox/radiobutton */
		...
		if (attr) {
			if ((value = get_tag_attribute(attr, "accel-key"))) {
				...
				if ((value = get_tag_attribute(attr, "accel-mods"))) {
					...
					g_snprintf(accel_path, sizeof(accel_path), "<%i>/%i", rand(), rand());
					custom_accel = TRUE;
#ifdef DEBUG_CONTENT
...
void widget_menuitem_save(variable *var)
{
	...
	if (GTK_IS_CHECK_MENU_ITEM(var->Widget)) {
		...
		g_warning( "%s(): Save not implemented for this widget.",
			__func__);
	}
...
static void widget_menuitem_input_by_command(variable *var, char *command)
{
	...
	if (GTK_IS_CHECK_MENU_ITEM(var->Widget)) {
		...
		g_warning( "%s(): <input> not implemented for this widget.",
			__func__);
	}
...
static void widget_menuitem_input_by_file(variable *var, char *filename)
{
	...
	if (GTK_IS_CHECK_MENU_ITEM(var->Widget)) {
		...
		g_warning( "%s(): <input file> not implemented for this widget.",
			__func__);
	}
