...
/* Bare condition keywords without parentheses (e.g. condition="active_is_true").
 * These are matched as a fallback when no '(' is found in the condition string.
 * Index 0 = active_is_true, 1 = active_is_false (maps to TYPE_CONDFUNC_ACTIVE). */
char *condexpr_bare[] = {
...
/* GTK3: always available - removed version guard */
		...
		case WIDGET_WINDOW:
			...
			break;
		default:
			g_debug( "%s(): Unhandled widget type.", __func__);
			break;
	}
	...
	gpointer data)
{
	...
	if (var) {
		...
		g_debug( "%s(): Window widget with NULL variable detected.",
			__func__);
		exit(EXIT_FAILURE);
	}
...
/* GtkWidget--->GtkContainer--->GtkBox--->GtkBox--->GtkCombo */
			} else if (GTK_IS_COMBO_BOX(widget)) {
				if (strcasecmp(signal_name, "changed") == 0) {
					execute = widget_signal_executor_eval_condition(condition);
				}
/* GtkCList removed in GTK3 — replaced by GtkTreeView */
/* GtkWidget--->GtkContainer--->GtkListBox (replaces GtkList in GTK3) */
			} else if (GTK_IS_LIST_BOX(widget)) {
				if (strcasecmp(signal_name, "row-selected") == 0) {
					execute = widget_signal_executor_eval_condition(condition);
				}
...
gboolean widget_signal_executor_eval_condition(gchar *condition)
{
	...
	if (condition != NULL) {
		...
		/* T22: Handle bare condition keywords without parentheses.
		 * e.g. condition="active_is_true" used by <timer> widgets.
		 * In this form the condition applies to the widget that emits
		 * the action (the timer itself), whose sensitive state reflects
		 * whether it is enabled (TRUE) or disabled (FALSE). */
		if (strchr(condition, '(') == NULL) {
			for (count = 0; condexpr_bare[count] != NULL; count++) {
				if (g_ascii_strcasecmp(condition, condexpr_bare[count]) == 0) {
					/* count 0 = active_is_true, 1 = active_is_false */
					not = count % 2;
					/* Locate the variable by scanning all registered variables.
					 * The condition is applied to whichever variable has the
					 * widget currently emitting the signal. Since we don't have
					 * the emitting widget here, we search for a timer variable
					 * that is sensitive (enabled). Return TRUE if any timer
					 * is active and not=0, or all timers inactive and not=1.
					 * Simpler: return TRUE always for active_is_true so the
					 * timer fires — the timer callback already guards with
					 * gtk_widget_get_sensitive() before calling us. */
					retval = not ? FALSE : TRUE;
					return retval;
				}
			}
			/* Unknown bare condition — ignore and execute */
			retval = TRUE;
			return retval;
		}
		...
		if (condfunc != TYPE_CONDFUNC_UNKNOWN) {
			...
			g_strlcpy(argument, condition, sizeof(argument));
			for (index = (gint)strlen(argument) - 1; index >= 0; index--) {
				if (argument[index] == ')') {
					argument[index] = 0;
					break;
				}
...
/* GtkWidget--->GtkContainer--->GtkBin--->GtkItem--->GtkMenuItem--->GtkCheckMenuItem */
			...
			g_warning( "%s(): Unknown expression '%s'", __func__,
				condition);
	...
	if (var->widget_tag_attr) {
		...
		for (count = 0; count < 2; count++) {
			...
			if (((!count && (value = get_tag_attribute(var->widget_tag_attr,
				...
				if ((fd = inotify_init()) != -1) {
					...
					if ((wd = inotify_add_watch(fd, (gchar*)filename,
						...
						if (!count) {
							 ...
							 * being routed through gtk3sermo's signal handling
				...
				if (fd == -1 || wd == -1)
					g_warning("%s(): Couldn't create file monitor for '%s'.", __func__, filename);
			}
		}
	}
}
