...
void action_closewindow(GtkWidget *widget, char *string)
{
	...
	if (existing != NULL && existing->Widget != NULL) {
		...
		/* If we are closing the last window then we can exit gtk3sermo */
...
void action_launchwindow(GtkWidget *widget, char *string)
{
	...
	if (var->Type != WIDGET_TIMER) {
		 ...
		 * (see Issue16 on the gtk3sermo website) */
		if (instruction_counter == 0) {
			...
			g_warning( "%s(): It's not possible to launch %s at this time: \
try launching after the originating window has fully loaded.",
				__func__, string);
		}

	} else {
		g_warning( "%s(): It's not possible to launch %s from within a timer: \
...
void action_exitprogram(GtkWidget *widget, char *string)
{
	...
	if (string[0] == '=') {
		...
		/* T13 fix: escape backslashes and double-quotes in the exit
		 * string so that  EXIT="<value>"  is safe to eval in a shell. */
	...
	if ((var = variables_get_by_name(string))) {
		...
		switch (response) {
			...
			case GTK_RESPONSE_OK: {
				/* GTK4: gtk_file_chooser_get_filename() removed —
				 * use gtk_file_chooser_get_file() + g_file_get_path() */
				GFile *gfile = gtk_file_chooser_get_file(
...
int action_append(GtkWidget *widget, char *string)
{
	...
	if (regexec(&twoparams, string, 3, matched, 0) != 0) {
		g_warning("%s(): Insert action needs two parameters.", __func__);
		return FALSE;
	}
...
int execute_action(GtkWidget *widget, const char *command, const char *type)
{
	...
	if (!function_signals_block) {
		...
		switch (t) {
			...
			case CommandPulse:
				variables_pulse(command_string);
				break;

			case CommandStart:
				variables_spinner_start(command_string);
				break;

			case CommandStop:
				variables_spinner_stop(command_string);
				break;
