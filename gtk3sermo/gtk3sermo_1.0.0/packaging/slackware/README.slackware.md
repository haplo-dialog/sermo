<div align="right"><sub><code>maj :     2026-08-18 12:40:07 · par claude</code></sub></div>
<div align="right"><sub><code>réindex : —</code></sub></div>

# Packaging Slackware — gtk3sermo

## Construction du paquet

```bash
# Placer l'archive source
cp gtk3sermo-1.0.0.tar.gz /tmp/

# Lancer le SlackBuild
cd packaging/slackware/
sudo sh gtk3sermo.SlackBuild

# Résultat
ls /tmp/gtk3sermo-1.0.0-x86_64-1_haplo.tgz
```

## Installation

```bash
sudo installpkg /tmp/gtk3sermo-1.0.0-x86_64-1_haplo.tgz
# ou
sudo upgradepkg --install-new /tmp/gtk3sermo-1.0.0-x86_64-1_haplo.tgz
```

## Dépendances (à installer manuellement)

Slackware ne gère pas les dépendances automatiquement.
Vérifier que les bibliothèques suivantes sont présentes :
- gtk3
- vte291

---

