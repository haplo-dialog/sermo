...
> [v1.1.2](https://gitlab.com/haplo-dialog/sermo/-/releases/v1.1.2) avec leurs
...
Les paquets sont joints à la release [**v1.1.2**](https://gitlab.com/haplo-dialog/sermo/-/releases/v1.1.2), avec leurs
...
|---|---|---|
| `gtk3sermo` 1.1.2-1 | `/usr/bin/gtk3sermo` | aucun |
| `gtk4sermo` 1.1.2-1 | `/usr/bin/gtk4sermo` | aucun |
| `gtksermo` 1.1.2-1 | `/usr/bin/gtkdialog` | **avec `gtkdialog` et `gtk3dialog`** |
...
> un, remplacez-le** par le paquet correspondant de la `v1.1.2`.
>
> La release `v1.0.0-4` porte, elle, les trois correctifs : ses paquets sont sains,
> simplement remplacés par ceux de la `v1.1.2`.
...
cd sermo/gtk3sermo/gtk3sermo_1.1.2        # ou gtk4sermo/gtk4sermo_1.1.2
...
- Manuels utilisateur/développeur (`gtk3sermo/gtk3sermo_1.1.2/MANUEL_*.md`).
- Pages de manuel `gtk3sermo(1)` et référence XML `haplo-dialog-xml(5)`.
- Manuels Texinfo (`gtk3sermo/gtk3sermo_1.1.2/doc/`).
